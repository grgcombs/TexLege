...
- (void)routeClass:(Class)class toResourcePath:(NSString*)resourcePath forMethodName:(NSString*)methodName {
	...
	if ([classRoutes objectForKey:methodName]) {
		[NSException raise:@"RestKit-Pre-existingRoute" format:@"A route has already been registered for class '%@' and HTTP method '%@'", className, methodName];
	}
...
- (NSString*)resourcePathForObject:(NSObject<RKObjectMappable>*)object method:(RKRequestMethod)method {
	...
	[NSException raise:@"RestKit-NoRoutablePath" format:@"Unable to find a routable path for object of type '%@' for HTTP Method '%@'", className, methodName];
...
- (NSObject<RKRequestSerializable>*)serializationForObject:(NSObject<RKObjectMappable>*)object method:(RKRequestMethod)method {
    ...
    NSUInteger propertyCount = [elementsAndProperties count];
    NSUInteger relationshipCount = [relationships count];
    NSUInteger entryCount = propertyCount + relationshipCount;
