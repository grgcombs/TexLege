...
@property (nonatomic,copy) NSString *infoItemId;
...
- (instancetype)initWithIdentifier:(nonnull NSString *)infoItemId
                          ...
                          duration:(NSTimeInterval)duration
{
    ...
    if (SLFTypeNonEmptyStringOrNil(infoItemId))
        dict[SLFInfoItemKeys.identifier] = infoItemId;
...
- (void)setDictionaryRepresentation:(NSDictionary *)dictionary
{
    ...
    self.infoItemId = SLFTypeNonEmptyStringOrNil(dictionary[SLFInfoItemKeys.identifier]);
...
- (BOOL)isValid
{
    return (SLFTypeNonEmptyStringOrNil(self.infoItemId)
...
- (id)copyWithZone:(NSZone *)zone
{
    ...
    if (self.infoItemId)
        copy.infoItemId = [self.infoItemId copyWithZone:zone];
    if (self.title)
...
- (instancetype)initWithCoder:(NSCoder *)decoder
{
    ...
    @try {
        if ([decoder containsValueForKey:SLFInfoItemKeys.identifier])
            self.infoItemId = [decoder decodeObjectOfClasses:allowedString forKey:SLFInfoItemKeys.identifier];
...
- (void)encodeWithCoder:(NSCoder *)encoder
{
    ...
    if (self.infoItemId)
        [encoder encodeObject:self.infoItemId forKey:SLFInfoItemKeys.identifier];
    if (self.title)
