...
const static CGFloat SLFUndefinedHue = -1.0f;

const SLFColorHSBComponents SLFColorInvalidHSBComponents = {
    .hue = -1,
    .saturation = -1,
    .brightness = -1,
    .alpha = 0
...
const SLFColorRGBComponents SLFColorInvalidRGBComponents = {
    .red = -1,
    .green = -1,
    .blue = -1,
    .alpha = 0
...
BOOL SLFColorIsValidRGBComponents(SLFColorRGBComponents rgb) {
    SLFColorRGBComponents invalidRGB = SLFColorInvalidRGBComponents;
...
BOOL SLFColorIsValidHSBComponents(SLFColorHSBComponents hsb) {
    SLFColorHSBComponents invalidHSB = SLFColorInvalidHSBComponents;
...
@interface NSString (SLFUtils)

- (unsigned int)slfColorHexValue;

@end

@implementation NSString (SLFUtils)
...
@implementation UIColor (SLFUtils)

#if 0

#pragma mark - Color Constants for Convenience

+ (UIColor *)slfCommonGrayValue        //102
{
    static dispatch_once_t onceToken;
    static UIColor *color;
    dispatch_once(&onceToken, ^{
        color = [UIColor colorWithWhite:102./255.f alpha:1];
    });
    return color;
}
...
{
    SLFColorRGBComponents rgb = [self rgbComponents];
...
+ (NSString *)slfHexStringByRemovingInvalidCharacters:(NSString *)hexString
{
    if (!SLFTypeNonEmptyStringOrNil(hexString))
        return nil;
...
+ (UIColor *)colorWithHexString:(NSString *)hexString
{
    ...
    hexString = [self slfHexStringByRemovingInvalidCharacters:hexString];
	...
	CGFloat red = [[hexString substringWithRange:NSMakeRange(0, 2)] slfColorHexValue] / 255.0f;
	CGFloat green = [[hexString substringWithRange:NSMakeRange(2, 2)] slfColorHexValue] / 255.0f;
	CGFloat blue = [[hexString substringWithRange:NSMakeRange(4, 2)] slfColorHexValue] / 255.0f;
	CGFloat alpha = [[hexString substringWithRange:NSMakeRange(6, 2)] slfColorHexValue] / 255.0f;
...
- (UIColor *)colorByInterpolatingToColor:(UIColor*)secondColor withRatio:(double)ratio
{
    return [self colorByBlendingWithColor:secondColor ratio:ratio blendAlpha:YES];
}
...
+ (UIColor *)interpolatedColorForValue:(id)value colors:(NSArray *)colors boundaries:(NSArray *)boundaries gradient:(BOOL)useGradient
{
    ...
    for (NSInteger i=1; i < boundaries.count; i++) 
    {
        if ([value compare:boundaries[i]] == NSOrderedAscending) 
        {
            ...
            return [firstColor colorByInterpolatingToColor:secondColor withRatio:ratio];
        }
    }
...
{
    SLFColorHSBComponents hsb = [self hsbComponents];

    hsb.hue *= hue;
    hsb.saturation *= saturation;
    hsb.brightness *= brightness;

    SLFColorRGBComponents rgb = [UIColor SLFColorRGBComponentsForHSB:hsb];
...
- (UIColor*)highlight {
    return [self multiplyHue:1 saturation:.5 brightness:1.2];
}

- (UIColor*)shadow
{
    return [self multiplyHue:1 saturation:.6 brightness:0.6];
}
...
- (UIColor *)colorWithBrightness:(CGFloat)brightness alpha:(CGFloat)alpha
{
	...
	hsb.brightness = brightness;
    hsb.alpha = alpha;
    return [UIColor SLFColorForHSBComponents:hsb];
}
...
{
	return (self.luminance > 0.8f) ? [UIColor blackColor] : [UIColor whiteColor];
}
...
// Pick two more colors such that all three are equidistant on the color wheel
...
- (NSArray*)analogousColorsWithStepAngle:(CGFloat)stepAngle pairCount:(NSInteger)pairs 
{
	...
	for (NSInteger i = 1; i <= pairs; ++i) 
	{
        ...
        SLFColorHSBComponents hsb1 = hsb;
        hsb1.hue = h1;
        hsb1.alpha = a;
        UIColor *color1 = [UIColor SLFColorForHSBComponents:hsb1];

        SLFColorHSBComponents hsb2 = hsb;
        hsb2.hue = h2;
        hsb2.alpha = a;
        UIColor *color2 = [UIColor SLFColorForHSBComponents:hsb2];
...
+ (SLFColorHSBComponents)SLFColorHSBComponentsForRGB:(SLFColorRGBComponents)rgb 
{
	SLFColorHSBComponents result;
...
+ (SLFColorRGBComponents)SLFColorRGBComponentsForHSB:(SLFColorHSBComponents)hsb
{
	SLFColorRGBComponents result;
    result.alpha = hsb.alpha;

	if (hsb.hue == SLFUndefinedHue || hsb.saturation == 0)
	{
        // achromatic (grey)
...
{
	return [UIColor SLFColorHSBComponentsForRGB:[self rgbComponents]];
}

+ (UIColor *)SLFColorForRGBComponents:(SLFColorRGBComponents)rgb 
{
    return [UIColor colorWithRed:rgb.red green:rgb.green blue:rgb.blue alpha:rgb.alpha];
}

+ (UIColor *)SLFColorForHSBComponents:(SLFColorHSBComponents)hsb 
{
    SLFColorRGBComponents rgb = [self SLFColorRGBComponentsForHSB:hsb];
    return [UIColor colorWithRed:rgb.red green:rgb.green blue:rgb.blue alpha:rgb.alpha];
}
...
{
    SLFColorHSBComponents hsl = [self SLFColorHSLComponents];

    hsl.hue *= hue;
    hsl.saturation *= saturation;
    hsl.brightness *= lightness;

    SLFColorRGBComponents rgb = [self SLFColorRGBComponentsForHSL:hsl];
    return [UIColor colorWithRed:rgb.red green:rgb.green blue:rgb.blue alpha:rgb.alpha];
}

- (SLFColorHSBComponents)SLFColorHSLComponents 
{
    SLFColorRGBComponents rgb = [self rgbComponents];
    SLFColorHSBComponents hslResult;
...
- (SLFColorRGBComponents)SLFColorRGBComponentsForHSL:(SLFColorHSBComponents)hsl 
{
    SLFColorRGBComponents rgbResult;
...
UIColor *SLFColorWithRGBShift(UIColor *color, int offset) {
    return [color colorWithRGBShift:offset];
}
